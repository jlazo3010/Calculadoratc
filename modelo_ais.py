...
def ejecutar_modelo_ais(nombre_muestra=None):
        ...
        for (pkg in packages_to_install) {{
            tryCatch({{
                if (pkg == "xgboost") {{
                    ...
                    install.packages("https://cran.r-project.org/src/contrib/Archive/xgboost/xgboost_1.4.1.1.tar.gz", repos = NULL, type = "source")
                }} else {{
    ...
    try:
        ...
        exit_code = process.wait(timeout=600)  # Aumentado a 10 minutos
